package com.teamcos.modularsystems.manager;

import com.teamcos.modularsystems.utilities.block.DummyBlock;
import com.teamcos.modularsystems.utilities.block.DummyEUBlock;
import com.teamcos.modularsystems.utilities.block.DummyIOBlock;
import com.teamcos.modularsystems.utilities.block.DummyRFBlock;
import net.minecraft.block.Block;

public class ApiBlockManager {
    public static DummyBlock dummyBlock;
    public static DummyIOBlock dummyIOBlock;
    public static DummyRFBlock dummyRFBlock;
    public static DummyEUBlock dummyEUBlock;
    public static Block furnaceOverlay;
    public static Block smelteryOverlay;
    public static Block fluidTank;
}
