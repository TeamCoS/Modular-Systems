...
public class GuiModularFurnace extends GuiBase<ContainerModularFurnace> {
    ...
    public void addRightTabs(GuiTabCollection tabs) {
        if (core != null) {
            ...
            furnaceInfoSpeed.add(new GuiComponentText(String.format("%.2f", (-1 * (((core.getValues().getEfficiency() + 200) / 200) - 1)) != 0 ? -1 * (((core.getValues().getEfficiency() + 200) / 200) - 1) : 0.00)  + "x", 15, 58, (-1 * (((core.getValues().getEfficiency() + 200) / 200) - 1)) > 0 ? 0x19A319 : (-1 * (((core.getValues().getEfficiency() + 200) / 200) - 1)) == 0 ? 0x000000 : 0x801A00));
