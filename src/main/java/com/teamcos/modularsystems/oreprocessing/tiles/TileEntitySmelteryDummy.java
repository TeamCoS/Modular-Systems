...
public class TileEntitySmelteryDummy extends ModularTileEntity implements ISidedInventory
{
    public int slot = 4;
    int coreX;
    int coreY;
    ...
    public TileEntitySmelteryDummy()
    {
    }
    ...
    public void updateEntity()
    {
        if (getCore() == null) {
            ...
            if (worldObj.getBlock(xCoord, yCoord, zCoord) instanceof BlockSmelteryDummyIO && !worldObj.isRemote && slot == 2) {
                for(int i = 0; i < ForgeDirection.VALID_DIRECTIONS.length; i++) {
                    ...
                    {
                        if(worldObj.getTileEntity(check.x, check.y, check.z) instanceof TileEntitySmelteryDummy)
                        {
                            TileEntitySmelteryDummy dummy = (TileEntitySmelteryDummy)worldObj.getTileEntity(check.x, check.y, check.z);
                        ...
                        else if(worldObj.getTileEntity(check.x, check.y, check.z) instanceof TileEntitySmelteryCore)
                        {
    ...
    public void setCore(TileEntitySmelteryCore core)
    {
        coreX = core.xCoord;
        coreY = core.yCoord;
        coreZ = core.zCoord;
    ...
    public TileEntitySmelteryCore getCore() {
        TileEntity te = worldObj.getTileEntity(coreX, coreY, coreZ);
        if (te instanceof TileEntitySmelteryCore) {
            return (TileEntitySmelteryCore) te;
        } else {
            return null;
        }
    }
