...
class GuiFurnaceCore(player: EntityPlayer, tile: TileEntityFurnaceCore) extends
    ...
    override def addRightTabs(tabs: GuiTabCollection): Unit = {
        if (core != null) {
            ...
            furnaceInfoSpeed += new GuiComponentText(String.format("%.2f", if ((-1 * (((core.values.speed + 200) / 200) - 1)) != 0) (-1 * (((core.values.speed + 200) / 200) - 1)) * 100 else 0.00) + "%", 15, 33, if ((-1 * (((core.values.speed + 200) / 200) - 1)) > 0) 0x5CE62E else if ((-1 * (((core.values.speed + 200) / 200) - 1)) == 0) 0x000000 else 0xE62E00)
